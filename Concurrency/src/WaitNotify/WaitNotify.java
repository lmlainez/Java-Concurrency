    ...
    private static class ThreadSafeQueue {
        ...
        public synchronized void add(MatricesPair matricesPair) {
            while (queue.size() == CAPACITY) {
                try {
                    //The queue will just wait in there and it won't produce more matrices in the queue if it is at
                    //capacity. It'll wait until ThreadSafeQueue.notify()/notifyAll() to add a matrixPair.
                    wait();
            ...
            queue.add(matricesPair);
            isEmpty = false;
            //Notifies any waiting Consumers that there are available elements in the queue to consume.
            notify();
        }
        ...
        public synchronized MatricesPair remove() {
            ...
            while (isEmpty && !isTerminate) {
                try {
                    //If not matrices are available but not over (fast consumers), the consumers
                    //ThreadSafeQueue.notify()/notifyAll()
                    wait();
                } catch (InterruptedException e) {
                }
            }
            ...
            // We check if after removing the element we are under capacity. At this point the producer is waiting
            // (add method) for the queue to give it access, if so, we need to wake up the producer(s),
            // so the matrix is added. If there is more than one producer, as the method is synchronized, only one will
            // add a value there.
